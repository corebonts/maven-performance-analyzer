...
describe("parser and analyzer", () => {
  ...
  it("surefire log", () => {
    const content = readFileSync(
      __dirname + "/testfiles/mavenSurefire.log",
      "utf8",
    );
    ...
    expect(
      durationSumForPlugin(result.mavenPlugins, "maven-surefire-plugin"),
    ).toEqual(434);
  ...
  it("log without timestamps", () => {
    const content = readFileSync(
      __dirname + "/testfiles/logWithoutTimestamps.log",
      "utf8",
    );
  ...
  it("log with simple timestamps", () => {
    const content = readFileSync(
      __dirname + "/testfiles/logWithSimpleTimestamps.log",
      "utf8",
    );
    ...
    expect(
      durationSumForPlugin(result.mavenPlugins, "maven-javadoc-plugin"),
    ).toEqual(71000);
    ...
    expect(result.modules).toEqual([
      {
        compiledSources: 478,
        compiledTestSources: 450,
        copiedResources: 4,
        copiedTestResources: 8,
  ...
  it("mutilthreaded build", () => {
    const content = readFileSync(
      __dirname + "/testfiles/mavenSurefireParallel.log",
      "utf8",
    );
    ...
    expect(threads).toEqual(
      expect.arrayContaining([
        "mvn-builder-surefire",
        "mvn-builder-surefire-api",
        "mvn-builder-surefire-report-parser",
      ]),
    );
    const duration = result.mavenPlugins
      .filter(
        (r) =>
          r.plugin === "maven-resources-plugin" &&
          r.thread === "mvn-builder-surefire-grouper",
      )
  ...
  it("guava log", () => {
    const content = readFileSync(
      __dirname + "/testfiles/guavaBuildJenkinsTimestamps.log",
      "utf8",
    );
    ...
    expect(durationSumForModule(result.mavenPlugins, "guava-parent")).toEqual(
      3869,
    );
    expect(durationSumForModule(result.mavenPlugins, "guava-testlib")).toEqual(
      361975,
    );
  ...
  it("guava log parallel", () => {
    const content = readFileSync(
      __dirname + "/testfiles/guavaBuildParallel.log",
      "utf8",
    );
  ...
  function durationSumForPlugin(
    result: readonly MavenPluginStats[],
    plugin: string,
  ): number {
    return result
